"""
Semantic memory store for knowledge base (docs, code, persona, etc.).
"""
...
class SemanticMemoryStore:
    """
    Store for semantic memories (knowledge base: docs, code, persona, facts, etc.).
    ...
    def add_chunk(self, chunk: "MemoryChunk") -> None:
        ...
        if chunk.embedding is None:
            raise ValueError("MemoryChunk must have an embedding to be added to semantic store")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to save chunk to semantic store: {e}") from e
    ...
    def _get_chunk(self, chunk_id: str) -> "MemoryChunk | None":
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load chunk from semantic store: {e}") from e
    ...
    def load_all(self) -> list["MemoryChunk"]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load chunks from semantic store: {e}") from e
